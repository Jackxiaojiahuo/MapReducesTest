 ...
 * @Author: Jack Zhou
 * @Description:
 * @Date: Created in 21:43 2018/11/26
 */
public class CollectTask extends TimerTask {
    ...
    public void run() {
         ...
         *  --获取需要采集的文件
         *  --移动这些文件到一个待上传临时目录
         *  --遍历待上传目录中个文件,逐一传输到HDFS的目标路径,同事将传输完成的文件移动到备份目录
         */
        try {
            Properties props = PropertyHodelLazy.getProps();
            ...
            // 将要采集的文件移动到待上传临时目录
            File toUploadDir = new File(props.getProperty(Constants.LOG_TOUPLOGD_DIR));
